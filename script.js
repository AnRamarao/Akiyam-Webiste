...
/** Derive the WebP optimized path from an image path (e.g., foo-opt.jpeg -> foo-opt.webp). */
function webpSource(imgPath) {
  return imgPath.replace(/\.(jpe?g|png)$/i, '.webp');
}

/** Create a person card HTML string with <picture> for WebP support. */
function personCardHTML(member) {
  const webp = webpSource(member.img);
  return `
    <picture>
